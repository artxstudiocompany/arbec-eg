...
export function EngineeringIntro({
  ...
  useGSAP(
    () => {
      ...
      const s = dir === "rtl" ? -1 : 1;
      const items = gsap.utils.toArray<HTMLElement>("[data-core-reveal]", ref.current);
      gsap.fromTo(
        items,
  ...
  return (
    <div ref={ref} className="relative">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
        ...
        <h2 className="mt-6 text-ar-display text-fg">
          <span data-core-reveal className="block">{titleLine1}</span>
          <span data-core-reveal className="block bg-gradient-to-r from-fg via-fg to-accent-300 bg-clip-text text-transparent">
            {titleLine2}
          </span>
        </h2>
        <p
